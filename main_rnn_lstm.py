...
def l2_loss(pred, label):
    loss = torch.nn.functional.mse_loss(pred, label, reduction='mean')  # , size_average=True
    return loss
...
def main():
    ...
    for epoch_idx in range(total_epoch):
        ...
        if epoch_idx%10==0:
            ...
            torch.save(model.state_dict(), "{}/KOSPI_LSTM/checkpoint_{:0>3}.ckpt".format(checkpointdir, epoch_idx))
            logger.add_scalar('Test/Loss', eval_loss, epoch_idx)
            # eval_plots()
            # plt.savefig("{}rnn/KOSPI_rnn_epoch{}.png".format(plotdir, epoch_idx))
